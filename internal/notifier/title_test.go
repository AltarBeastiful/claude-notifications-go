...
// TestTitlePipeline_FromHookMessage runs the same sequence SendDesktop uses to
// build the title, starting from the message format hooks.sendNotifications
...
func TestTitlePipeline_FromHookMessage(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sessionName, _, _ := extractSessionInfo(tt.message)

			title := "✅ Task Completed"
			if identity := titleIdentity(tt.cwd, sessionName); identity != "" {
